...
app.post('/user/new', async (request, reply) => {
  const { username, age, credenciais: { password }, tipo, nome, turmas, redacoes } = request.body;
  try {
    const user = new User({ username, age, credenciais: { password }, tipo, nome, turmas, redacoes });
...
app.post('/redacao/new', async (request, reply) => {
  ...
  try {
    const redacao = new Redacao({ aluno, professor, turma, titulo, status, data });
...
// Iniciar servidor
app.listen({ port: 4000, host: '192.168.0.33' }, (err, address) => {
  if (err) {
    app.log.error(err);
    process.exit(1);
  }
